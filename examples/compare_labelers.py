...
pl.close('all')
import itertools
compare_code = 0 # 0: with gt, 1: among themselves, 2: with all consensus, 3: with at least 2 consensus
label_name = ['regions/natalia_active_regions_nd.zip','regions/lindsey_active_regions_nd.zip','regions/sonia_active_regions_nd.zip','regions/ben_active_regions_nd.zip']
...
for count,par in enumerate(params):
    print(os.path.join('/mnt/ceph/neuro/labeling/',par[0],'projections/correlation_image.tif'))
    c_img = cm.load(os.path.join('/mnt/ceph/neuro/labeling/',par[0],'projections/correlation_image.tif'))
    result = dict()
    if compare_code == 0:
    ...
    for region_pairs in iterlabels:
        ...
        try:
            if compare_code == 0:
                roi_nat = nf_read_roi_zip(os.path.join('/mnt/ceph/neuro/labeling/',par[0],consensus_name),c_img.shape)
                roi_lin = nf_read_roi_zip(os.path.join('/mnt/ceph/neuro/labeling/',par[0],region_pairs),c_img.shape)

            else:
                roi_nat = nf_read_roi_zip(os.path.join('/mnt/ceph/neuro/labeling/',par[0],region_pairs[0]),c_img.shape)                
                roi_lin = nf_read_roi_zip(os.path.join('/mnt/ceph/neuro/labeling/',par[0],region_pairs[1]),c_img.shape)
...
#        pl.figure(figsize=(30,20))
        ...
        if plot_movie:
            ...
            font = {'family' : 'Helvetica',
                    ...
                    'size'   : 10}
            pl.rc('font', **font)
            pl.savefig(os.path.join('/mnt/ceph/neuro/DataForPublications/Labeling/images/spatial_comparison',fig_name))
            pl.pause(1)
            pl.savefig(os.path.join('/mnt/ceph/neuro/DataForPublications/Labeling/images/spatial_comparison',fig_name[:-4]+'.png'))
            pl.figure(figsize=(30,20))
...
for count,ref_name in enumerate(['ben','lindsey','sonia','natalia']):
    ...
    all_values = []
    if compare_code==0:
        iterations_on = [[(r,consensus_name[8:-12],a[8:-15],b['performance']['f1_score']) for a,b in k.iteritems()] for r,k in results.iteritems()]
    elif compare_code ==1:
        iterations_on = [[(r,a[0][8:-12],a[1][8:-12],b['performance']['f1_score']) for a,b in k.iteritems()] for r,k in results.iteritems()]
...
#df = pd.DataFrame(vals, index=cols, columns=index)       
if compare_code==0:
    iterations_on = [[(r,consensus_name[8:-12],a[8:-15],b['performance']['f1_score'],b['performance']['precision'],b['performance']['recall']) for a,b in k.iteritems()] for r,k in results.iteritems()]
elif compare_code ==1:
    iterations_on = [[(r,a[0][8:-12],a[1][8:-12],b['performance']['f1_score'],b['performance']['precision'],b['performance']['recall']) for a,b in k.iteritems()] for r,k in results.iteritems()]     
all_values = []
for fl in iterations_on: #[[(r,a[0][8:-12],a[1][8:-12],b['performance']['f1_score'],b['performance']['precision'],b['performance']['recall']) for a,b in k.iteritems()] for r,k in results.iteritems()]:    
    all_values+=fl
    
    
df = pd.DataFrame(all_values, columns = ('filename','labeler_1','labeler_2','F1','precision','recall')) 
...
#    pl.figure()
    labels.append(key)
    grp.sort_values('date', ascending=True)
    a = pl.hist(grp['F1'],label=key,cumulative = True,normed = 1, bins = 10,alpha = 0)
