...
inputs = [{'fname':'/mnt/ceph/neuro/labeling/neurofinder.03.00.test/images/final_map/Yr_d1_498_d2_467_d3_1_order_C_frames_2250_.mmap', 'gSig' : [8,8]},
          ...
          {'fname': '/mnt/ceph/neuro/labeling/Jan-AMG_exp3_001/images/final_map/Yr_d1_512_d2_512_d3_1_order_C_frames_115897_.mmap', 'gSig':[7,7]}
          {'fname': '', 'gSig':[]}]
...
for dc in inputs[:]:
    ...
    print(fname)
    gSig = dc['gSig']
    gt_file = os.path.join(os.path.split(fname )[0], os.path.split(fname )[1][:-4] + 'match_masks.npz')
    #% LOAD DATA
...
#%%
training_files = [os.path.join(dp, f) for dp, dn, filenames in os.walk('/mnt/ceph/neuro/labeling/') for f in filenames if 'set_minions.npz' in f]
...
#        coms = com(scipy.sparse.coo_matrix(A_matched[()]), dims[0], dims[1])
        ...
        traces_gt = traces_gt + list(YrA_matched+C_matched) + list(C_unmatched+YrA_unmatched) +list(C_negative + YrA_negative)
