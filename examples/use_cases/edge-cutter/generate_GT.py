...
m_orig  = cm.Movie(images)
    ...
    predictions,final_crops = evaluate_components_CNN(A_gt,dims,gSig,model_name = 'use_cases/CaImAnpaper/cnn_model', isGPU = False)
    #%%
    thresh = .95
    ...
    #idx_included = idx_components_cnn[np.arange(50)] # neurons that are displayed
        ...
        #%
        fitness,exceptionality,sd_r,md = cm.components_evaluation.compute_event_exceptionality(traces_gt[idx_included],robust_std=False,N=5,use_mode_fast=False)
        fitness_d,exceptionality_d,sd_r_d,md_d = cm.components_evaluation.compute_event_exceptionality(np.diff(traces_gt[idx_included],axis=1),robust_std=False,N=3,use_mode_fast=False)
...
#            return ain.reshape(dims,order = 'F'), cin, cin_res 
        ...
        #    img_temp = m_res[count].copy().astype(np.float32)/max_mov
            possibly_active = np.where((exceptionality[:,count-count_start:count].min(-1)<-10) & (exceptionality[:,count-count_start:count].min(-1)>=-25))[0]
            super_active = np.where((exceptionality[:,count-count_start:count].min(-1)<-25))[0]
...
#            cms_2 = [np.array(scipy.ndimage.center_of_mass(np.reshape(a.toarray(),dims,order = 'F'))).astype(np.int) for a in  A_gt.tocsc()[:,idx_included[super_active]].T]
            ...
            cms_2 = np.array([np.minimum(cms,dims-half_crop) for cms in cms_2]).astype(np.int) 
...
#                img_temp = cv2.rectangle(img_temp,(cm_[1]-half_crop, cm_[0]-half_crop),(cm_[1]+half_crop, cm_[0]+half_crop),0)
                image_no_neurons[ cm_[0]-half_crop:cm_[0]+half_crop,cm_[1]-half_crop:cm_[1]+half_crop] = np.nan
            
            for cm__ in cms_1:
                cm_=cm__[::]
#                img_temp = cv2.rectangle(img_temp,(cm_[1]-half_crop, cm_[0]-half_crop),(cm_[1]+half_crop, cm_[0]+half_crop),.5)
                all_dubious_crops.append(image_orig[cm_[0]-half_crop:cm_[0]+half_crop,cm_[1]-half_crop:cm_[1]+half_crop])
...
#                img_temp = cv2.rectangle(img_temp,(cm_[1]-half_crop, cm_[0]-half_crop),(cm_[1]+half_crop, cm_[0]+half_crop),1)
                all_pos_crops.append(image_orig[cm_[0]-half_crop:cm_[0]+half_crop,cm_[1]-half_crop:cm_[1]+half_crop])
...
#                    img_temp = cv2.rectangle(img_temp,(py-half_crop, px-half_crop),(py+half_crop, px+half_crop),1)
                    all_neg_crops.append(image_orig[px-half_crop:px+half_crop,py-half_crop:py+half_crop])
    ...
    try:
        with np.load(fl) as ld:
            ...
            for class_id,pos in enumerate([all_pos_crops, all_neg_crops]):#, all_dubious_crops]):
                ...
                total_crops += [cv2.resize(ain/np.linalg.norm(ain),(patch_size ,patch_size)) for ain in pos]
...
#%%
crd = cm.utils.visualization.plot_contours(A_gt.tocsc()[:,idx_included], Cn, thr=.99)
