...
idx_xy=None
base_name='Yr'
name_new=cm.save_memmap_each(fnames_map, dview=dview,base_name=base_name, resize_fact=(1, 1, downsample_factor), remove_init=0,idx_xy=idx_xy,add_to_movie=add_to_movie,border_to_0=border_to_0)
name_new.sort()
print(name_new)
#%%
if len(name_new)>1:
...
downs = .2
m_els.resize(1,1,downs).play(fr = 30, gain = 5,magnification=1, offset = add_to_movie)          
...
#%% save results
np.savez(os.path.join(os.path.split(fname_new)[0], os.path.split(fname_new)[1][:-4]+'results_analysis_3.npz'), Cn=Cn, A=A.todense(), C=C, b=b, f=f, YrA=YrA, sn=sn, d1=d1, d2=d2, idx_components=idx_components, idx_components_bad=idx_components_bad)
#%%
pl.subplot(1, 2, 1)
