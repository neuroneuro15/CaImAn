...
for (x0, x1, y0 ,y1, _) in sliding_window_new(m[0], overlaps, strides):
     ...
     mc_init, shifts_init, template_init, corrs_init = m[:initbatch,x0:x1,y0:y1].copy().motion_correct(max_shifts[0],max_shifts[1])
     mc_init -= img_min
...
for (x0, x1, y0 ,y1, _) in sliding_window_new(m[0], overlaps, strides):
     print([x0,y0])
     mc, shifts, template, corrs = m[:,x0:x1,y0:y1].copy().motion_correct(max_shifts[0],max_shifts[1])
     Cns_all.append(mc.local_correlations(eight_neighbours = True,swap_dim  = False))
...
#%%
np.savez('demo_zebra_variables', Cns_all=Cns_all,templates_all=templates_all,shifts_all=shifts_all,corrs_all=corrs_all,imgs_norm = imgs_norm , Cns = Cns , base_names = base_names)    
#%%
...
#    pl.subplot(3,5,count+1)    
    A, C, b, f, YrA, sn = cnm.Ab.tocsc()[:,:-1], cnm.C_on[:-1], cnm.Ab.tocsc()[:,-1:], cnm.C_on[-1:], cnm.noisyC[:-1], cnm.sn
