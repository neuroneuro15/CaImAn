...
#%%
params_movie = {'fname': ['example_movies/msCam13.tif'],
               'niter_rig': 1,
...
params_movie = {'fname': ['example_movies/msCam1.tif'],
               'niter_rig': 1,
               'gSig_filt': [9, 9], # size of the spatial filters is gSig_filt*3
               'max_shifts': (20, 20),  # maximum allow rigid shift
               'splits_rig': 10,  # for parallelization split the movies in  num_splits chuncks across time
               # if none all the splits are processed and the Movie is saved
               'num_splits_to_process_rig': None,
               ...
               'splits_els': 10,  # for parallelization split the movies in  num_splits chuncks across time
               # if none all the splits are processed and the Movie is saved
               'num_splits_to_process_els': [5, None],
               'upsample_factor_grid': 4,  # upsample factor to avoid smearing when merging patches
               # maximum deviation allowed for patch with respect to rigid
               # shift
               'max_deviation_rigid': 2,
...
m_orig.resize(1, 1, downsample_ratio).play(
    gain=1, offset=offset_mov, fr=30, magnification=2)
...
min_mov = np.array([cm.motion_correction.low_pass_filter_space(m_,params_movie['gSig_filt']) for m_ in cm.load(fname[0], subindices=range(400))]).min()
mc_list = []
new_templ = None
...
# TODO: needinfo how the classes works
    mc = MotionCorrect(each_file, min_mov,
                       ...
                       shifts_opencv=True, nonneg_movie=True, gSig_filt = params_movie['gSig_filt'])
...
cm.Movie(np.array(mc.templates_rig)).play(
    fr=10, gain=1, magnification=2, offset=offset_mov)
# %% plot rigid shifts
pl.close()
...
m_rig.resize(1, 1, downsample_ratio).play(
    gain=1, offset=offset_mov * .25, fr=30, magnification=2, bord_px=bord_px_rig)
# %%
...
m_els.resize(1, 1, downsample_ratio).play(
    gain=1, offset=0, fr=30, magnification=2, bord_px=bord_px_els)
# %% local correlation
...
cm.concatenate(
    [m_orig.resize(1, 1, downsample_factor) + offset_mov, m_rig.resize(1, 1, downsample_factor), m_els.resize(
        1, 1, downsample_factor)], axis=2).play(fr=60, gain=1, magnification=1, offset=0)
