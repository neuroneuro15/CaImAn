...
for count in range(count_start,T):
    ...
    if full_fov:
        image_orig = locate(image_orig[:,:,None], rectangle = True)[1].squeeze()
    else:
        ...
        for cm__ in cms_total:
            cm_=cm__[::]                
            ain =  img_avg[cm_[0]-half_crop:cm_[0]+half_crop, cm_[1]-half_crop:cm_[1]+half_crop]            
            avg_crops.append(cv2.resize(ain/np.linalg.norm(ain),(patch_size ,patch_size)))
